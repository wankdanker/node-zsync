 ...
 *   * intermediary - send intermediate snapshots
 */
...
function push(opts, cb) {
	...
	each(opts, function (dataset, next) {
		...
		debug('push(): requesting diff between %s:%s and %s:%s', req.sourceHost || 'local', req.source, req.destinationHost || 'local', req.destination);
			...
			if (!req.fromSnap && result.destinationSnapshots.length) {
				if (opts.fallbackDestroyDestination) {
					...
					return destroy({
						...
						, sourceHost : req.destinationHost
					}, function (err) {
						if (err) {
							return maybeContinue(err);
						}
				...
				else if (opts.fallbackRenameDestination) {
					debug('push(): no common snapshots found, destination has snapshots, fallbackRenameDestination is a go. Initiating rename.');
				...
				debug('push(): no source snapshot available for an incremental send.');
				if (req.intermediary !== false) {
					debug('push(): Forcing a replication stream.');
					req.replication = true;
				}
				else {
					debug('push(): not forcing a replication stream because intermediary is false');
				}
			}
