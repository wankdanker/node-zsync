			...
			if (opts.destinationHost) {
				receiveStream.pipe(gatherJSON(function (err, data) {
					...
					if (err) {
						...
						// I'm not sure if it's ok to just set sendStream.ended = true here for all
						// cases, but I'm encountering for EPIPE errors ultimately, the maybeFinish
						// never succeeds because .on('close') and .on('end') are never called.
						if (err.code === 'EPIPE') {
